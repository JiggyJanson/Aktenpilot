...
function renderCases() {
  ...
  document.getElementById("caseList").innerHTML = filtered.length ? filtered.map(c => `<article class="case-row ${c.id === state.selectedCaseId ? "selected" : ""}" data-case-id="${c.id}"><div class="case-row-top"><span class="case-name">${caseTitleHtml(c)}</span><span class="status ${c.status}">${statusLabel(c.status)}</span></div><small>${esc(c.reference || c.party || "Keine zusätzlichen Angaben")}</small></article>`).join("") : empty("Keine passenden Fälle");
